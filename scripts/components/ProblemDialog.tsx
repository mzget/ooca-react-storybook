...
import { PaddingBox, PrimaryButton, TextArea } from "../StyleComponents/Styles";
import { PrimaryDialogButton, SecondaryDialogButton, DialogHeader, LableDialog, ListDialogItem } from './DialogContent';
...
export class ProblemDialog extends React.Component<{
    ...
    setAnchor(positionElement: string, position: any) {
                    ...
                    <div style={{ paddingTop: '15px', marginBottom: '25px', padding: '0px 40px 0px 40px' }}>
                        <div style={
                            ...
                            {[
                                ...
                                return (
                                    <InputRadio key={index} text={value.text}
                                        onClick={() => this.setAnchor('vertical', value.options)}
                                        checked={this.state.anchorOrigin.vertical === value.options}
                                    />
                                );
                            })}
                        </div>
                        {
                            (this.state.anchorOrigin.vertical === options.other) ?
                                <TextArea
                                    hintText={WordingInfo.Recommend[_isLocal]}
