...
export class FeedbackDialog extends React.Component {
    ...
    render() {
        ...
        <Dialog contentStyle={{ maxWidth: '500px', minWidth: '400px' }} bodyStyle={{ textAlign: 'center', padding: 0 }} actionsContainerStyle={{ padding: 0 }} actions={[]} modal={true} open={this.state.open} onRequestClose={() => { this.setState({ open: false }); }}>
          <BottomButtonDialog isProvider={_isProvider} header={MSGSteateInfo.Feedback[_isLocal]} content={<TextArea placeholder={WordingInfo.Recommend[_isLocal]} onChange={(e) => {
            FeedbackInfo.feedback = e.target.value;
        }}/>} submitText={WordingInfo.Submit[_isLocal]} skipText={WordingInfo.Skip[_isLocal]} onSubmitClick={() => {
            SendFeedback();
