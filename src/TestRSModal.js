...
import { MSGSteateInfo, FeedbackInfo } from './Localized/MessageInfo.js';
...
const InputRadio = (props) => {
    return (<div className="control">
      <label className="radio">
        <input type="radio" name={props.name} value={props.value} checked={props.checked} onClick={props.onClick} style={{ display: 'inline-block' }}/>
...
class TestRSModal extends React.Component {
    constructor(props) {
        super(props);
        this.handleOpen = () => { this.setState({ open: true }); };
        this.handleClose = () => { this.setState({ open: false, MessageState: MSGSteate.Quality, anchorOrigin: { anchorOrigin: '' } }); };
    ...
    ActiveOnState(_state) {
        const TextArea = styled.textarea.attrs({ cols: 30, rows: 4, className: 'textarea full-width bottom-space' }) `outline: none; -webkit-tap-highlight-color:transparent;`;
        // console.log("ActiveOnState: ",_state);
        switch (_state) {
            case MSGSteate.Quality:
                {
                    return (<ThumbUpDialog isLocal={_isLocal} isProvider={_isProvider} handMSGState={this.handMSGState}/>);
                }
            case MSGSteate.Feedback:
                {
                    return (<FeedbackDialogWithTheme _isLocal={_isLocal} _isProvider={_isProvider} SendFeedback={this.SendFeedback.bind(this)} handMSGState={this.handMSGState.bind(this)}/>);
                }
            case MSGSteate.Problem:
                {
                    return (<ProblemDialogWithTheme _isLocal={_isLocal} _isProvider={_isProvider} SendFeedback={this.SendFeedback.bind(this)} handMSGState={this.handMSGState.bind(this)}/>);
                }
            case MSGSteate.SendMSG: {
                return (<IndeterminateDialog isModal={true} isOpen={true} message={MSGSteateInfo.SendMSG[_isLocal]}/>);
            }
            case MSGSteate.Thank:
                {
                    return (<ThankyouDialogWithTheme isLocal={_isLocal} isProvider={_isProvider} handCallBack={this.handCallBack.bind(this)} handleClose={() => {
